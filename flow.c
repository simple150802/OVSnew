...
miniflow_extract(struct dp_packet *packet, struct miniflow *dst)
{
    ...
    ovs_be32 nw_options[3] = {0,0,0}; /*Tuan Anh mod*/
    ...
    if (OVS_LIKELY(dl_type == htons(ETH_TYPE_IP))) {
        ...
        memcpy(nw_options, nh->ip_options, sizeof(nw_options)); //Tuan Anh mod
        //nw_options[3] = nh->ip_options; /*Tuan Anh mod*/
        nw_tos = nh->ip_tos;
        nw_ttl = nh->ip_ttl;
                               ...
                               const struct flow *flow)
{
    ...
    WC_MASK_FIELD(wc, nw_options); /*Tuan Anh mod*/
